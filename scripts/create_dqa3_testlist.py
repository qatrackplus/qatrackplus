    ...
    try:
        with transaction.atomic():
            ...
            if mode == 'testpack':
                ...
                fname = f'dqa3-qcpump-{beam.lower()}-test-list.tpk'
                json.dump(tp, open(fname, 'w'), indent=2)
                print(f"Wrote '{test_list.name}' Test Pack to {fname}")
                raise Rollback("Rollback so we don't actually save the tests")
            else:
                domain = Site.objects.get_current().domain
                url = f'{settings.HTTP_OR_HTTPS}://{domain}{test_list.get_absolute_url()}'
                print(f"Created '{test_list.name}' Test List ({url})")
