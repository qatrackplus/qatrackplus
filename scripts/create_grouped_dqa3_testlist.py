...
from django.utils.text import slugify as django_slugify  # noqa: E402

from qatrack.qa import models
from qatrack.qa.testpack import create_testpack
from qatrack.qa.utils import get_internal_user  # noqa: E402
    ...
    try:
        with transaction.atomic():
            ...
            if mode == 'testpack':
                ...
                print(f"Wrote '{parent_test_list.name}' Test Pack to {fname}")
                raise Rollback("Rollback so we don't actually save the tests")
            else:
                domain = Site.objects.get_current().domain
                url = f'{settings.HTTP_OR_HTTPS}://{domain}{parent_test_list.get_absolute_url()}'
                print(f"Created '{parent_test_list.name}' Test List ({url})")
