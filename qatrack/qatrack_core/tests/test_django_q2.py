...
class TestDjangoQ2BasicFunctionality(TestCase):
    ...
    def test_recurring_schedule(self):
        ...
        schedule = Schedule.objects.create(
            func="qatrack.qatrack_core.tasks.run_periodic_scheduler", schedule_type=Schedule.MINUTES, minutes=5
        )
    ...
    def test_schedule_execution(self):
        ...
        schedule = Schedule.objects.create(
            func="qatrack.qatrack_core.tasks.run_periodic_scheduler", schedule_type=Schedule.ONCE, next_run=next_run
        )
...
class TestDjangoQ2Integration(TestCase):
    ...
    def test_notification_scheduling(self):
        ...
        notice = QCSchedulingNotice.objects.create(
            recipients=recipients, notification_type=QCSchedulingNotice.DUE, time="0:00"
        )
    ...
    def test_report_scheduling(self):
        ...
        saved_report = SavedReport.objects.create(
            title="Test Report", report_type="qc", created_by=user, modified_by=user
        )

        # Create a test report schedule
        report_schedule = ReportSchedule.objects.create(
            report=saved_report, time=timezone.now().time(), created_by=user, modified_by=user
        )
