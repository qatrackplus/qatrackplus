...
class TestRelativeDates:
    ...
    def test_next_week(self):
        start = start_of_day(timezone.datetime(2020, 1, 5, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 1, 11, tzinfo=ZoneInfo("America/Toronto")))
        r = relative_dates("next week", self.now)
        assert r.start() == start
        assert r.end() == end

    def test_next_week_sat(self):
        pivot = timezone.datetime(2020, 1, 11, 11, 38, tzinfo=ZoneInfo("America/Toronto"))
        start = start_of_day(timezone.datetime(2020, 1, 12, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 1, 18, tzinfo=ZoneInfo("America/Toronto")))
        r = relative_dates("next week", pivot)
        assert r.start() == start
        assert r.end() == end

    def test_next_week_sun(self):
        pivot = timezone.datetime(2020, 1, 12, 11, 38, tzinfo=ZoneInfo("America/Toronto"))
        start = start_of_day(timezone.datetime(2020, 1, 19, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 1, 25, tzinfo=ZoneInfo("America/Toronto")))
        r = relative_dates("next week", pivot)
        assert r.start() == start
        assert r.end() == end

    def test_next_month(self):
        start = start_of_day(timezone.datetime(2020, 2, 1, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 2, 29, tzinfo=ZoneInfo("America/Toronto")))
        r = relative_dates("next month", self.now)
        assert r.start() == start
        assert r.end() == end

    def test_next_month_first_day(self):
        pivot = timezone.datetime(2020, 1, 1, 11, 38, tzinfo=ZoneInfo("America/Toronto"))
        start = start_of_day(timezone.datetime(2020, 2, 1, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 2, 29, tzinfo=ZoneInfo("America/Toronto")))
    ...
    def test_next_month_last_day(self):
        pivot = timezone.datetime(2020, 1, 31, 11, 38, tzinfo=ZoneInfo("America/Toronto"))
        start = start_of_day(timezone.datetime(2020, 2, 1, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 2, 29, tzinfo=ZoneInfo("America/Toronto")))
        r = relative_dates("next month", pivot)
        assert r.start() == start
    ...
    def test_next_year(self):
        start = start_of_day(timezone.datetime(2021, 1, 1, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2021, 12, 31, tzinfo=ZoneInfo("America/Toronto")))
        r = relative_dates("next year", self.now)
        assert r.start() == start
        assert r.end() == end

    def test_this_week(self):
        start = start_of_day(timezone.datetime(2019, 12, 29, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 1, 4, tzinfo=ZoneInfo("America/Toronto")))
    ...
    def test_this_week_sun(self):
        pivot = timezone.datetime(2020, 1, 5, 11, 38, tzinfo=ZoneInfo("America/Toronto"))
        start = start_of_day(timezone.datetime(2020, 1, 5, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 1, 11, tzinfo=ZoneInfo("America/Toronto")))
        r = relative_dates("this week", pivot)
        assert r.start() == start
    ...
    def test_this_year(self):
        start = start_of_day(timezone.datetime(2020, 1, 1, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 12, 31, tzinfo=ZoneInfo("America/Toronto")))
    ...
    def test_this_year_dec_31(self):
        pivot = timezone.datetime(2020, 12, 31, 11, 38, tzinfo=ZoneInfo("America/Toronto"))
        start = start_of_day(timezone.datetime(2020, 1, 1, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 12, 31, tzinfo=ZoneInfo("America/Toronto")))
        r = relative_dates("this year", pivot)
        assert r.start() == start
    ...
    def test_last_7_days(self):
        start = start_of_day(timezone.datetime(2019, 12, 26, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 1, 2, tzinfo=ZoneInfo("America/Toronto")))
        r = relative_dates("last 7 days", self.now)
        assert r.start() == start
        assert r.end() == end

    def test_last_30_days(self):
        start = start_of_day(timezone.datetime(2019, 12, 3, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 1, 2, tzinfo=ZoneInfo("America/Toronto")))
    ...
    def test_last_365_days(self):
        start = start_of_day(timezone.datetime(2019, 1, 2, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 1, 2, tzinfo=ZoneInfo("America/Toronto")))
    ...
    def test_this_month(self):
        start = start_of_day(timezone.datetime(2020, 1, 1, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2020, 1, 31, tzinfo=ZoneInfo("America/Toronto")))
        r = relative_dates("this month", self.now)
        assert r.start() == start
        assert r.end() == end

    def test_last_week(self):
        start = start_of_day(timezone.datetime(2019, 12, 22, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2019, 12, 28, tzinfo=ZoneInfo("America/Toronto")))
        r = relative_dates("last week", self.now)
        assert r.start() == start
        assert r.end() == end

    def test_last_week_sat(self):
        pivot = timezone.datetime(2020, 1, 4, 11, 38, tzinfo=ZoneInfo("America/Toronto"))
        start = start_of_day(timezone.datetime(2019, 12, 22, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2019, 12, 28, tzinfo=ZoneInfo("America/Toronto")))
    ...
    def test_last_month(self):
        start = start_of_day(timezone.datetime(2019, 12, 1, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2019, 12, 31, tzinfo=ZoneInfo("America/Toronto")))
    ...
    def test_last_year(self):
        start = start_of_day(timezone.datetime(2019, 1, 1, 11, 38, tzinfo=ZoneInfo("America/Toronto")))
        end = end_of_day(timezone.datetime(2019, 12, 31, tzinfo=ZoneInfo("America/Toronto")))
