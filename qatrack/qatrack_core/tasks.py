...
def _schedule_periodic_task(function, task_name, interval_min=None, next_run=None, schedule_type=Schedule.MINUTES):
    ...
    if next_run is None:
        ...
        next_run = now.replace(minute=0, second=0, microsecond=0) + timezone.timedelta(
            seconds=int(interval_min * 60 / 2.0)
        )
    ...
    try:
        ...
        sch.save()
        logger.info(f'{function} next run updated to {next_run}')
        ...
        logger.info(f'{function} schedule created and next run set to {next_run}')
...
def run_periodic_scheduler(model, log_name, handler, time_field='time', recurrence_field='recurrence'):
    ...
    logger.info(f'Running {log_name} task at {now} for notices between {start_time} and {end_time}')

    # first narrow down notices to those supposed to run in this 15 minute block
    instances = model.objects.filter(
        **{
            f'{time_field}__gte': start_time.strftime('%H:%M'),
            f'{time_field}__lte': end_time.strftime('%H:%M'),
        }
    )
    ...
    for instance in instances:
        ...
        logger.info(
            f'Occurences for {model._meta.model_name} {instance.id}: {occurences} (between {start_today} & {end_today})'
        )
