...
def get_time_display(dt):
    if not isinstance(dt, timezone.datetime):
        raise ValueError(f'{dt} is not a valid datetime')
    ...
    def edit_se_attachments(self, service_event):
        for idx, f in enumerate(self.request.FILES.getlist('se_attachments')):
            Attachment.objects.create(
                attachment=f,
                comment=f'Uploaded {timezone.now()} by {self.request.user.username}',
                label=f.name,
            ...
            if delete and not is_new:
                ...
                if current_p.set_quantity_current():
                    messages.add_message(
                        ...
                        level=messages.INFO,
                        message=f'Part {str(current_p)} is low ({current_p.quantity_current} left in stock).',
                    )
                if initial_p:
...
class DeleteServiceEvent(DeleteView, FormView, PermissionRequiredMixin):
    ...
    def form_invalid(self, form):
        messages.add_message(self.request, messages.WARNING, f'Could not delete service event {form.instance.id}')
        return reverse('sl_dash')
...
class ServiceEventsByStatusList(ServiceEventsBaseList):
    ...
    def get_page_title(self, *args, **kwargs):
        ...
        return f'Service Events - Status: {status.name}'
...
class ServiceEventsByUnitList(ServiceEventsBaseList):
    ...
    def get_page_title(self, *args):
        ...
        return f'Service Events For {unit}'
...
class ReturnToServiceQAForEventList(ReturnToServiceQABaseList):
    def get_page_title(self):
        ...
        description = f'{se.unit_service_area} - {format_datetime(se.datetime_service)} {qa_tags.service_status_label(se.service_status)}'
...
class TLISelect(UTCInstances):
    ...
    def get_page_title(self):
        try:
            ...
            return f'Select a {utc.name} instance'
...
class ServiceEventScheduleList(BaseListableView):
    ...
    order_fields = {
        ...
        'unit_service_area__unit__name': f'unit_service_area__unit__{settings.ORDER_UNITS_BY}',
        'due_date': 'due_date',
    }
...
def service_log_report(request, pk):
    ...
    base_opts = {
        ...
        'title': f'Service Event {se.pk} - {se_date}',
        'include_signature': False,
        'visible_to': [],
    }

    report_opts = {
        'datetime_service': f'{se_date} - {se_date}',
