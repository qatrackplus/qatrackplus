...
class ServiceEventPersonnelSummaryReport(BaseReport):
    ...
    def get_filename(self, report_format):
        return '{}.{}'.format(slugify(self.name or 'service-event-personnel-summary'), report_format)

    def get_unit_service_area__unit_details(self, val):
        units = umodels.Unit.objects.filter(pk__in=val).select_related('site')
        return (
            'Unit(s)',
            ', '.join('{}{}'.format(f'{unit.site.name} - ' if unit.site else '', unit.name) for unit in units),
        )
    ...
    def format_user(self, username, first_name, last_name):
        if first_name or last_name:
            return '{} ({})'.format(username, ', '.join(x for x in [last_name, first_name] if x))
    ...
    def collect_group_linker_instances(self):
        ...
        gli_users = (
            models.GroupLinkerInstance.objects.values_list(
                'user',
                'user__username',
                'user__first_name',
                'user__last_name',
        ...
        for user_id, uname, fname, lname in gli_users:
            ...
            group_linkers = (
                models.GroupLinker.objects.filter(
                    group__user=user_id,
                )
    ...
    def collect_hours(self):
        ...
        tp_hours = (
            hours.exclude(third_party=None)
            .values(
