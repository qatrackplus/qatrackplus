    ...
    def filter_form_valid(self, filter_form):
        ...
        if nse > self.MAX_SES:
            filter_form.add_error(
                '__all__',
                'This report can only be generated with %d or fewer Service Events'
...
class ServiceEventSummaryReport(ServiceEventReportMixin, BaseReport):
    ...
    description = mark_safe(
        _l('This report includes a summary of all Service Events from a given' 'time period for selected units')
    )
    ...
    def get_filename(self, report_format):
        return '{}.{}'.format(slugify(self.name or 'service-event-summary'), report_format)
