...
class DueDatesReportMixin(filters.ServiceEventScheduleFilterDetailsMixin):
    ...
    def get_context(self):
        ...
        for site in sites:
            ...
            ).order_by('due_date', f'unit_service_area__unit__{settings.ORDER_UNITS_BY}')

            for schedule in schedules:
                window = schedule.window()
                if window:
                    window = f'{format_as_date(window[0])} - {format_as_date(window[1])}'
...
class NextScheduledServiceEventsDueDatesReport(DueDatesReportMixin, BaseReport):
    ...
    def get_filename(self, report_format):
        return '{}.{}'.format(slugify(self.name or _('next-due-dates-for-sl-report')), report_format)
...
class DueAndOverdueServiceEventScheduleReport(DueDatesReportMixin, BaseReport):
    ...
    def get_filename(self, report_format):
        return '{}.{}'.format(slugify(self.name or _('due-and-overdue-sl-report')), report_format)
