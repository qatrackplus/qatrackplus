...
class ServiceEventDetailsReport(ServiceEventReportMixin, BaseReport):
    ...
    description = mark_safe(
        _l('This report includes details of all Service Events from a given' 'time period for selected units')
    )
    ...
    def get_filename(self, report_format):
        return '{}.{}'.format(slugify(self.name or 'service-event-details'), report_format)
        ...
        for site in sites:
            ...
            for se in self.get_ses_for_site(self.filter_set.qs, site):
                ...
                for h in se.hours_set.all():
                    u = h.user.username if h.user else f'{str(h.third_party)} ({h.third_party.vendor.name})'
        ...
        for site, ses in context['sites_data']:
            for se in ses:
                ...
                for group, members in se['group_linkers']:
                    group_members.append('{}={}'.format(group, ','.join(m.split('(')[0] for m in members)))
                group_members = '\n'.join(group_members)

                hours = ','.join('{}={}'.format(*ut) for ut in se['hours'])

                rts_qc = []
                for utc, wc, link in se['rts_qc']:
                    rts_qc.append('{},{},{}'.format(utc, wc or _('Not completed'), link or _('Not completed')))
                rts_qc = '\n'.join(rts_qc)

                rts_comments = []
                for comments in se['rts_comments']:
                    rts_comments.append('{},{},{}'.format(*comments))
                rts_comments = '\n'.join(rts_comments)

                parts = ','.join('{}={}={}'.format(*p) for p in se['parts'])
