...
class FaultDetailsReport(FaultReportMixin, BaseReport):
    ...
    description = mark_safe(_l('This report includes details of all Faults from a giventime period for selected units'))
    ...
    def get_filename(self, report_format):
        return '{}.{}'.format(slugify(self.name or 'fault-details'), report_format)
        ...
        for site, faults in context['sites_data']:
            for fault in faults:
                ...
                for comment in fault['comments']:
                    comments.append('{},{},{}'.format(*comment))
                comments = '\n'.join(comments)
