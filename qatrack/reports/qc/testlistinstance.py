...
class TestListInstanceSummaryReport(BaseReport):
    ...
    def filter_form_valid(self, filter_form):
        ...
        if ntlis > self.MAX_TLIS:
            filter_form.add_error(
                '__all__',
                'This report can only be generated with %d or fewer Test List '
    ...
    def get_filename(self, report_format):
        return '{}.{}'.format(slugify(self.name or 'test-list-instance-summary'), report_format)
    ...
    def get_unit_test_collection__unit_details(self, val):
        ...
        units = ('{} - {}'.format(u.site.name if u.site else _('Other'), u.name) for u in units)
    ...
    def get_context(self):
        ...
        sites = (
            self.filter_set.qs.order_by('unit_test_collection__unit__site__name')
            .values_list('unit_test_collection__unit__site', flat=True)
            .distinct()
        )
    ...
    def get_tlis_for_site(self, qs, site):
        ...
        tlis = (
            tlis.order_by(
                f'unit_test_collection__unit__{settings.ORDER_UNITS_BY}',
...
class TestListInstanceDetailsReport(BaseReport):
    ...
    def get_filename(self, report_format):
        return '{}.{}'.format(slugify(self.name or 'test-list-instance-details'), report_format)

    def get_unit_test_collection_details(self, val):
        utcs = models.UnitTestCollection.objects.filter(pk__in=val)
        return ('Unit / Test List', ', '.join(f'{utc.unit.name} - {utc.name}' for utc in utcs))

    def get_context(self):
        context = super().get_context()

        qs = (
            self.filter_set.qs.select_related(
                'created_by',
                'modified_by',
                'reviewed_by',
                'test_list',
