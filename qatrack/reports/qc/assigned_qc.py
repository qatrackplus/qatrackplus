...
class AssignedQCReport(filters.UnitTestCollectionFilterDetailsMixin, BaseReport):
    ...
    def get_filename(self, report_format):
        return '{}.{}'.format(slugify(self.name or 'qc-assignment-summary-report'), report_format)
...
class AssignedQCDetailsReport(filters.UnitTestCollectionFilterDetailsMixin, BaseReport):
    ...
    def filter_form_valid(self, filter_form):
        ...
        if nutc > self.MAX_UTCS:
            ...
            ) % {'max_num_unit_assignments': self.MAX_UTCS, 'num_unit_assignments': nutc}
    ...
    def get_filename(self, report_format):
        return '{}.{}'.format(slugify(self.name or 'qc-assignment-details-report'), report_format)
            ...
            for utc in self.get_utcs_for_site(self.filter_set.qs, site):
                ...
                for li in all_lists:
                    ...
                    for test in li.ordered_tests():
                        ...
                        include = (
                            (has_ref_or_tol and include_ref_tols == 'refs_tols_set')
                            or (not has_ref_or_tol and include_ref_tols == 'refs_tols_not_set')
                            or include_ref_tols == 'both'
                        )
                        if include:
