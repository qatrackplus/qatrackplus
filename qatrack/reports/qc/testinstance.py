...
class TestInstanceDetailsReport(BaseReport):
    ...
    def get_filename(self, report_format):
        return '{}.{}'.format(slugify(self.name or _('test-instance-details')), report_format)
    ...
    def organize_by_unit_test_date(self):
        ...
        unit_test_combos = list(
            ...
            .order_by(
                f'unit_test_info__unit__{settings.ORDER_UNITS_BY}',
        ...
        for unit_id, site_name, unit_name in unit_qs:
            units[unit_id] = '{} : {}'.format(site_name or _('Other'), unit_name)

        table = [[_('Date')]]

        for unit_id, test_id in unit_test_combos:
            table[-1].append(f'{units[unit_id]} - {tests[test_id]}')
