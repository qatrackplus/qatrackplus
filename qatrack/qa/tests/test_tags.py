...
class TestRefTolSpan(TestCase):
    ...
    def test_absolute(self):
        ...
        self.assertIn('{} L'.format(settings.TEST_STATUS_DISPLAY_SHORT['action']), result)
...
class TestToleranceForReference(TestCase):
    ...
    def test_bool(self):
        ...
        self.assertIn('{}: Yes'.format(settings.TEST_STATUS_DISPLAY['ok']), qa_tags.tolerance_for_reference(None, r))
    ...
    def test_multiple_choice(self):
        tol = models.Tolerance(type=models.MULTIPLE_CHOICE, mc_tol_choices='foo', mc_pass_choices='')
        assert '{}: foo'.format(settings.TEST_STATUS_DISPLAY['tolerance']) in qa_tags.tolerance_for_reference(tol, None)
