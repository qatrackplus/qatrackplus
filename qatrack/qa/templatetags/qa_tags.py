...
def reference_tolerance_span(test, ref, tol):
    ...
    if test.is_boolean():
        return mark_safe(f'<span title="Passing value = {ref.value_display()}">{ref.value_display()}</span>')

    if not tol:
        if ref:
            return mark_safe(f'<span title="No Tolerance Set">{ref.value_display()}</span>')
    ...
    if tol.type == models.MULTIPLE_CHOICE:
        ...
        return mark_safe(
            (
                '<span><abbr title="{} Values: {};  {} Values: {}; All other choices are failing">'
    ...
    if tol.type == models.ABSOLUTE:
        return mark_safe(
            '<span> <abbr title="({} L, {} L, {} H, {} H) = {} ">{}</abbr></span>'.format(
                tsds['action'],
                tsds['tolerance'],
                tsds['tolerance'],
                tsds['action'],
                str(tol).replace('Absolute', ''),
...
def tolerance_for_reference(tol, ref):
    ...
    if tol.type == models.MULTIPLE_CHOICE:
        params = (tsd['ok'], ', '.join(tol.pass_choices()), tsd['tolerance'], ', '.join(tol.tol_choices()))
        return mark_safe('<span>{}: {}</br>  {}: {}</br> All others fail</span>'.format(*params))

    tols = tol.tolerances_for_value(ref.value)
    for key in tols:
        tols[key] = '-' if tols[key] is None else f'{tols[key]:.4g}'
    tols['ok_disp'] = tsd['ok']
    tols['tol_disp'] = tsd['tolerance']
    tols['act_disp'] = tsd['action']
    return mark_safe(
        '<span>{ok_disp}: Between {tol_low} &amp; {tol_high}</br> '
        '{tol_disp} Between {act_low} &amp; {act_high}</br> '
        '{act_disp}: < {act_low} or > {act_high}</span>'.format(**tols)
    )
...
def history_display(history, unit, test_list, test, frequency=None):
    ...
    date_range = f'{format_as_date(start_date)} - {format_as_date(end_date)}'
...
def as_data_attributes(unit_test_collection):
    ...
    return ' '.join([f'data-{k}={v}' for k, v in list(attrs.items()) if v])
