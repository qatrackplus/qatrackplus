...
def send_qcreview_notice(notice_id, task_name=''):
    ...
    if notice:
        if not notice.send_required():
            logger.info(f'Send of QCReviewNotice {notice_id} requested, but no QC to notify about')
            return

        recipients = notice.recipients.recipient_emails()
        if not recipients:
            logger.info(f'Send of QCReviewNotice {notice_id} requested, but no recipients')
            return
    else:
        logger.info(f'Send of QCReviewNotice {notice_id} requested, but no such QCReviewNotice exists')
        return
    ...
    try:
        ...
        logger.info(f'Sent QCReviewNotice {notice_id} at {timezone.now()}')
        try:
        ...
        logger.exception(f'Error sending email for QCReviewNotice {notice_id} at {timezone.now()}.')
