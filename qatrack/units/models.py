...
class UnitType(models.Model):
    ...
    def __str__(self):
        """Display more descriptive name"""
        return '{}{}'.format(self.name, f' - {self.model}' if self.model else '')
...
class Unit(models.Model):
    ...
    def site_unit_name(self):
        return '{} :: {}'.format(_('Other') if not self.site else self.site.name, self.name)
    ...
    def get_potential_time(self, date_from, date_to):
        ...
        self_uat_set = self.unitavailabletime_set.filter(date_changed__range=[date_from, date_to]).order_by(
            'date_changed'
        )
...
class UnitAvailableTimeEdit(models.Model):
    ...
    def __str__(self):
        return f'{self.name} ({fmt_date(self.date)})'
...
class UnitAvailableTime(models.Model):
    ...
    def to_dict(self):
        return {
            'date_changed': f'{self.date_changed.day:02d}-{self.date_changed.month:02d}-{self.date_changed.year}',
